...
CONSTANTS = {
    "OSMOSIS": {
        "SYMBOLS": {
            ...
            "AXLWBNB": "WBNB", "EVMOS": "EVMOS", "STATOM": "stATOM", "STOSMO": "stOSMO", "LUNA": "LUNA",
        },
    ...
    "COIN_GECKO": {
        "SYMBOLS": {
            ...
            "STATOM": "stride-staked-atom", "STOSMO": "stride-staked-osmo", "LUNA": "terra-luna-2", "CANTO": "canto",
        },
...
def get_lp_prices_from_cswap(symbols):
    # Return empty response if no symbols
    if not symbols:
        return []
    ...
    # URL to retrieve the price of all tokens
    url = f"{STAT_URL}/api/v2/cswap/lp/prices"
    ...
    try:
        ...
        lp_symbol_prices_map = {}
        for token in response["data"]:
            lp_symbol_prices_map[token["symbol"]] = token["price"]

        return [lp_symbol_prices_map[symbol] if lp_symbol_prices_map.get(symbol, None) else 0  for symbol in symbols]
...
def main(symbols):
    ...
    try:
        ...
        # Get lp price from cswap
        result_cswap_lp_prices = get_lp_prices_from_cswap(symbols)

        # if lenghth of the results from all the sources is not same, then return 0
        if not len(result_coingecko) == len(result_osmosis) == len(result_cswap) == len(result_cswap_lp_prices):
            return ",".join("0" for i in range(len(symbols)))

        result = []
        for item in zip(result_coingecko, result_osmosis, result_cswap, result_cswap_lp_prices):
